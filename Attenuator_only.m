function attenuator_chain_optimizer()
    clc; clear;
    fprintf('--- Attenuator Chain Noise Optimizer ---\n');
    % Input number of attenuators
    n = input('Enter the number of attenuators: ');

    atts = struct('gain_dB', {}, 'NF_dB', {});

    % User enters attenuators
    for i = 1:n
        fprintf('Attenuator %d:\n', i);
        atts(i).gain_dB = input('  Enter Gain (in dB): ');  % should be negative for loss
        atts(i).NF_dB = input('  Enter Noise Figure (in dB): ');
    end
    ...
    gains_user = 10.^(arrayfun(@(x) x.gain_dB, atts) / 10);
    NFs_user = 10.^(arrayfun(@(x) x.NF_dB, atts) / 10);
    ...
    for p = 1:num_perms
        idx = perms_idx(p, :);
        gains = 10.^(arrayfun(@(x) atts(x).gain_dB, idx) / 10);
        NFs = 10.^(arrayfun(@(x) atts(x).NF_dB, idx) / 10);
    ...
    G_best_dB = sum(arrayfun(@(x) atts(x).gain_dB, best_order));
    ...
    for i = 1:n
        idx = best_order(i);
        fprintf('  Att %d → Gain: %.2f dB, NF: %.2f dB\n', i, atts(idx).gain_dB, atts(idx).NF_dB);
    end
    fprintf('Total Gain: %.2f dB\n', G_best_dB);
    fprintf('Total Noise Figure: %.2f dB\n', NF_best_dB);
    fprintf('Output Power (Pout): %.2f dBm\n', Pout_best);
...
%% function attenuator_configuration()
attenuator_chain_optimizer()
